...
//			System.out.println("当前map的大小=="+map.size());
	 ...
	 * @return
	 */
	public Bitmap getBitmapFromFile(Activity mActivity,String imageName,String path){
		Bitmap bitmap = null;
		if(imageName != null){
			File file = null;
			String real_path = "";
			try {
				if(ImageUtil.getInstance().hasSDCard()){
					real_path = ImageUtil.getInstance().getExtPath() + (path != null && path.startsWith("/") ? path : "/" + path);
				}else{
					real_path = ImageUtil.getInstance().getPackagePath(mActivity) + (path != null && path.startsWith("/") ? path : "/" + path);
				}
